describe('Models.GamePosition', () => {
    ...
    describe('constructor (from size)', () => {
        for (let sz: number = 2; sz <= 38; ++sz) {
            ...
            it('should support size ' + sz.toString(), () => {
                ...
                assert(position.schema[schemaLength - 1] == Models.GameMarks.BlackStone, "failed to find black stone at (" + (sz - 1).toString() + ", " + (sz - 1).toString() + ")");
            });
        }
    });
    ...
    let stoneMarks: Models.GameMarks[] = [
        Models.GameMarks.WhiteStone,
        Models.GameMarks.BlackStone,
    ];

    let decorativeMarks: Models.GameMarks[] = [
    ...
    function gameMarksToString(gameMarks: Models.GameMarks): string {
        let marks: string[] = [];
    ...
    describe('stone-equality comparison', () => {
        testCases.forEach((testCase: TestCase) => {
            describe(testCase.name, () => {
                ...
                for (let j = 0; j < stoneMarks.length; ++j) {
                    let mark: number = stoneMarks[j];
                    it("should not ignore mark '" + gameMarksToString(mark) + "'", () => {
                        ...
                        switch (mark) {
                            ...
                            default:
                                assert(false, "unknown mark '" + gameMarksToString(mark) + "'");
                                break;
                        }
                ...
                for (let j = 0; j < decorativeMarks.length; ++j) {
                    let mark: number = decorativeMarks[j];
                    it("should ignore decorative mark '" + gameMarksToString(mark) + "'", () => {
                        let clone = new Models.GamePosition(original);
